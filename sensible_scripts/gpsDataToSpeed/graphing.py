...
slider = RangeSlider(ax_slider, 'Data Range', 0, len(mycalc_data))
...
def update(val):
  ...
  ax.clear()
  ax.plot(x_values, y_values, color="blue", marker="o", markerfacecolor="red", markeredgecolor="red", markersize=1, label="Speed")
  ...
  x_shown_labels = ['' if i % 5 != 0 else str(end_index[i]) for i in range(len(end_index))]
